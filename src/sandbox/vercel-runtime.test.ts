...
describe('vercel-runtime', () => {
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    // Use fake timers to speed up exponential backoff delays
    vi.useFakeTimers();
  ...
  describe('executeAgentInSandbox', () => {
    ...
    describe('error handling (AC#4)', () => {
      it('should handle sandbox creation failure with SANDBOX_CREATION_FAILED', async () => {
        ...
        const resultPromise = executeAgentInSandbox(validInput);
      ...
      it('should handle timeout errors with SANDBOX_TIMEOUT code (AC#4)', async () => {
        const mockUpdate = vi.fn().mockResolvedValue({ ok: true });
        (WebClient as unknown as Mock).mockImplementation(() => ({
          chat: { update: mockUpdate },
        }));
        ...
        // Run execution with timer advancement for retry backoff
        const resultPromise = executeAgentInSandbox(validInput);
        // Advance timers through all retry backoffs (1s, 2s for 3 retries)
        await vi.runAllTimersAsync();
        const result = await resultPromise;

        expect(result.success).toBe(false);
        expect(result.error).toContain('timed out');
        // Verify SANDBOX_TIMEOUT error code produces correct user message
        expect(mockUpdate).toHaveBeenCalledWith(
          expect.objectContaining({
            text: 'Your request took too long. Please try a simpler question.',
          })
        );
      });
    ...
    describe('thread history handling', () => {
      it('should handle empty thread history', async () => {
        const inputWithEmptyHistory: SandboxExecutionInput = {
          ...validInput,
          threadHistory: [],
        };

        const result = await executeAgentInSandbox(inputWithEmptyHistory);

        expect(result.success).toBe(true);
        // Script should only contain the user message, no history
        const writeFilesCall = mockSandbox.writeFiles.mock.calls[0][0][0];
        const scriptContent = writeFilesCall.content.toString();
        expect(scriptContent).toContain(validInput.userMessage);
      });
      ...
      it('should parse User: and Assistant: prefixes in thread history', async () => {
        const inputWithPrefixes: SandboxExecutionInput = {
          ...validInput,
          threadHistory: [
            'User: What is 2+2?',
        ...
        await executeAgentInSandbox(inputWithPrefixes);

        const writeFilesCall = mockSandbox.writeFiles.mock.calls[0][0][0];
        const scriptContent = writeFilesCall.content.toString();

        // Prefixes should be stripped from content
        expect(scriptContent).toContain('What is 2+2?');
        expect(scriptContent).toContain('2+2 equals 4.');
        expect(scriptContent).toContain('Thanks!');
        // Roles should be properly set (check JSON structure)
        expect(scriptContent).toContain('"role":"user"');
        expect(scriptContent).toContain('"role":"assistant"');
      });
    });
  });
});
