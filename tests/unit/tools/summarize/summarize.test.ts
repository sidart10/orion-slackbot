...
import { summarize } from '@/tools/summarize/summarize.js';

// Mock the sub-modules
vi.mock('@/tools/summarize/summarize-thread.js', () => ({
...
vi.mock('@/tools/summarize/summarize-conversation.js', () => ({
...
describe('summarize', () => {
  ...
  describe('AC5: Handle all conversation types - Routing', () => {
    it('routes to thread summarization when thread URL provided', async () => {
      const { summarizeThread } = await import('@/tools/summarize/summarize-thread.js');
    ...
    it('routes to conversation summarization for channel requests', async () => {
      const { summarizeConversation } = await import('@/tools/summarize/summarize-conversation.js');
