...
// Hoist glob mock so it can be controlled in tests
const { mockGlob } = vi.hoisted(() => ({
  mockGlob: vi.fn().mockResolvedValue(['SKILL.md']),
}));
vi.mock('glob', () => ({
  glob: mockGlob,
}));

vi.mock('@/skills/api-client.js', () => ({
...
describe('skills/sync-service', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    // Reset glob mock to default (returns SKILL.md)
    mockGlob.mockResolvedValue(['SKILL.md']);
  });
  ...
  describe('syncSkills', () => {
    it('uploads new skills when cache is empty', async () => {
      ...
      const { loadSkillMetadata } = await import('@/skills/loader.js');
      const { SkillsApiClient } = await import('@/skills/api-client.js');
    ...
    it('uses cached skill IDs when content unchanged', async () => {
      ...
      // WHEN: Syncing skills
      const { syncSkills, _resetCacheForTests } = await import('@/skills/sync-service.js');
      _resetCacheForTests?.();
      const result = await syncSkills('test-trace-id');
  ...
  describe('hashSkillDirectory', () => {
    it('computes SHA-256 hash of skill directory contents', async () => {
      ...
      const { hashSkillDirectory } = await import('@/skills/sync-service.js');
  ...
  describe('loadSkillFiles', () => {
    it('loads all files from skill directory for upload', async () => {
      ...
      const { loadSkillFiles } = await import('@/skills/sync-service.js');
