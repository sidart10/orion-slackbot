...
vi.mock('@/agent/model-capabilities.js', () => ({
...
// Import after mocks
import { executeAgentLoop, type AgentLoopOptions, extractMarkdownLinks, formatToolDisplayName, summarizeToolInput } from '@/agent/loop.js';
  ...
  describe('source citations v2', () => {
    it('extracts markdown links from tool results and adds to sources', async () => {
      ...
      const urlSources = result.sources.filter(
        (s): s is import('@/agent/gather.js').ContextSource & { url: string } => typeof s.url === 'string'
      );
...
describe('extractFileIds', () => {
  ...
  it('extracts file_id from code_execution_tool_result content block', async () => {
    ...
    const { extractFileIds } = await import('@/agent/loop.js');
...
describe('Story 8.6: tool_search_tool_bm25 inclusion', () => {
  ...
  it('includes tool_search_tool_bm25 when deferred tools exist and tool search enabled (AC#1)', async () => {
    ...
    const { getToolDefinitions } = await import('@/agent/tools.js');
...
describe('Prompt Caching (Anthropic API)', () => {
  ...
  it('should separate base system prompt from dynamic context in content blocks', async () => {
    ...
    const gatherModule = await import('@/agent/gather.js');
