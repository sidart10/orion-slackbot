...
vi.mock('@/tools/memory/loader.js', () => ({
  ...
  // formatMemoriesForContext available but not imported in handler yet
}));
...
describe('Thread Started Handler', () => {
  ...
  beforeEach(async () => {
    ...
    const handlerModule = await import('@/slack/handlers/thread-started.js');
  ...
  describe('Memory Auto-Check (Story 5.3)', () => {
    ...
    // Note: formatMemoriesForContext is available but not used yet (context injection planned)
    // When implemented, add test: expect(formatMemoriesForContext).toHaveBeenCalled();
