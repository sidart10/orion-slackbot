...
const mockReplies = vi.fn().mockResolvedValue({
  ok: true,
  messages: [
    { user: 'U123', text: 'User: hello', ts: '1234567890.000000' },
    { bot_id: 'B123', text: 'Orion: hi there', ts: '1234567890.000001' },
  ],
...
// Mock Vercel Sandbox execution (Story 3.0)
...
vi.mock('../../src/observability/tracing.js', () => ({
  startActiveObservation: vi.fn(
    async (_ctx: unknown, fn: (trace: any) => any) => {
      const mockTrace = {
        id: 'test-trace-id',
...
describe('Slack API Route', () => {
  let handler: typeof import('../../api/slack.js').default;
  ...
  beforeEach(async () => {
    ...
    const module = await import('../../api/slack.js');
  ...
  describe('AC#3: Immediate Acknowledgment', () => {
    it('should send 200 response before starting async processing for app_mention events', async () => {
      ...
      const jsonCallOrder =
        (res.json as any).mock.invocationCallOrder?.[0] ?? 0;
